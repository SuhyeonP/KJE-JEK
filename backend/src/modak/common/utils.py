# Thanks for: https://gist.github.com/shello/efa2655e8a7bce52f273
...
# Your system may not support Unicode 7.0 characters just yet! So hipster.
UNICODE_VERSION = 6
...
def random_emoji(unicode_version: int = 6) -> Emoji:
    ...
    # Emoji 😄
    emoji_value = chr(ord(emoji_range[0]) + point_in_range)
    emoji_name = unicode_name(emoji_value, NO_NAME_ERROR).capitalize()
    emoji_codepoint = "U+{}".format(hex(ord(emoji_value))[2:].upper())

    return Emoji(value=emoji_value, codepoint=emoji_codepoint, name=emoji_name)
